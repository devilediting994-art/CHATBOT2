...
async def unclone_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_main = (BOT_TOKEN == next((t for t, r in clone_registry.items() if int(r.get("bot_id", 0)) == current_bot_id), None))
    # More reliable main-bot check: compare against configured main token.
    is_main = not any(int(r.get("bot_id", 0)) == current_bot_id for r in clone_registry.values())
